...
class Inventory {
  ...
  transferItem (targetInv, sourceID, keepOrig = false) {
    ...
    if (sourceID === 'gold') {
      ...
      if (isNaN(parseInt(sourceID))) {
        if (!keepOrig) { var item = this.remove(sourceID) } else { var item = this.get(sourceID) }
      } else {
  ...
  transferForGold (targetInv, sourceID, cost, sellFrac = 1, keepOrig = false) {
    ...
    if (typeof cost === 'undefined') {
      if (isNaN(parseInt(sourceID))) { var item = this.inv[sourceID] } else { var item = this.inv[parseInt(sourceID)] }
